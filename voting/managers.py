...
class VoteManager(models.Manager):
    ...
    def record_vote(self, obj, user, vote):
        ...
        """
        if vote not in range(settings.MIN_SCORE, settings.MAX_SCORE+1) + [0]:
            raise ValueError('Invalid vote (must be [%d,%d] or 0)' % (
                                settings.MIN_SCORE,
                                settings.MAX_SCORE)
                            )
        ctype = ContentType.objects.get_for_model(obj)
    ...
    def get_top(self, Model, limit=10, reversed=False):
        ...
        if reversed:
            having_sql = ' ORDER BY %(having_score)s ASC %(limit_offset)s'
        else:
            having_sql = ' ORDER BY %(having_score)s DESC %(limit_offset)s'
        query += having_sql % {
            'having_score': qn('score'),
            #'limit_offset': connection.ops.limit_offset_sql(limit),
            'limit_offset': 'LIMIT '+str(limit)+' OFFSET 0'#++' 0',
        }
