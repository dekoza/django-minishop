...
def vote_display(vote, arg=None):
    ...
    =========  ========================================================  =================
    ``1``      ``"Beginner,Intermediate,Experienced,Master,Hardcore"``   ``Beginner``
    ``2``      ``"Beginner,Intermediate,Experienced,Master,Hardcore"``   ``Intermediate``
    ``3``      ``"Beginner,Intermediate,Experienced,Master,Hardcore"``   ``Experienced``
    ``4``      ``"Beginner,Intermediate,Experienced,Master,Hardcore"``   ``Master``
    ``5``      ``"Beginner,Intermediate,Experienced,Master,Hardcore"``   ``Hardcore``
    =========  ========================================================  =================

    If no string mapping is given, "1","2","3", etc. will be used.

    Example usage::

        {{ vote|vote_display:"Beginner,Intermediate,Experienced,Master,Hardcore" }}
    """
    if arg is None:
        arg = ','.join([str(x) for x in range(settings.MIN_SCORE, settings.MAX_SCORE+1)])
    bits = arg.split(',')
    if len(bits) != (settings.MAX_SCORE - settings.MIN_SCORE + 1):
        return vote.vote # Invalid arg
    return bits[vote.vote-MIN_SCORE]
